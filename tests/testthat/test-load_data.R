...
json_file <- system.file("data-raw/national_community_child_health_database_(ncchd)_20240405T130125.json", package = "browseMetadata")
look_up_file <- system.file("data-raw/look_up.csv", package = "browseMetadata")
domains_file <- system.file("data-raw/domain_list_demo.csv", package = "browseMetadata")
...
test_that("load_data reads user-specified files correctly", {
  ...
  expect_equal(result$DomainListDesc, tools::file_path_sans_ext(basename(domains_file)))
})
