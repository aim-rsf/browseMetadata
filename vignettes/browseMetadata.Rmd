...
For installation, set-up and basic usage refer to the package [README.md](https://github.com/aim-rsf/browseMetadata/blob/main/README.md) file. 

This page provides more context on how to interpret package outputs and what to input into the R console when it prompts you for inputs.

## `browseMetadata()`

The json file contains information about the data asset (dataset), data class (table) and data element (variable). 

The metadata catalogue uses *Data asset* to mean *Dataset* (a collection of data, can contain multiple tables). *Data class* refers to *Table*, and *Data Element* refers to each *Variable* name within a table. 

See [here](https://github.com/aim-rsf/browseMetadata/tree/main/inst/outputs/), generated from a demo run. 

The BROWSE_table html summarises the dataset and each table in the dataset. The BROWSE_bar html, pasted below, is a simple bar plot summarising the dataset. The BROWSE_bar csv file contains the data used to make this bar plot. 
...
The height of the bar indicates the number of variables in that table - the ones with lots of variables (e.g. CHILD_TRUST) will take you longer to process when running `mapMetadata()`. 
